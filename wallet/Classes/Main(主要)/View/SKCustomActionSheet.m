...
@interface SKCustomActionSheet ()<UITableViewDelegate,UITableViewDataSource>
...
@implementation SKCustomActionSheet{
    
    SKCustomActionSheetItemClickHandle _clickHandle;
    SKCustomActionSheetItemClickHandle _dismissHandle;
...
+ (instancetype)actionSheetWithCancelTitle:(NSString *)cancelTitle alertTitle:(NSString *)alertTitle SubTitles:(NSString *)title,...NS_REQUIRES_NIL_TERMINATION {
    SKCustomActionSheet *sheet = [[SKCustomActionSheet alloc] initWithFrame:CGRectMake(0, 0, SKScreenWidth, SKScreenHeight)];
...
- (UIView *)tableView:(UITableView *)tableView viewForHeaderInSection:(NSInteger)section {
    ...
    if (section == 0) {
        ...
        if ([SKValidStr(self.alertTitle) length] == 0) {
            return nil;
        }
        UIView *view = [[UIView alloc] initWithFrame:CGRectMake(0, 0, SKScreenWidth, 60)];
        view.backgroundColor = [UIColor whiteColor];
        
        NSMutableAttributedString *string = [[NSMutableAttributedString alloc] initWithString:SKValidStr(self.alertTitle)];
        ...
        [string addAttribute:NSFontAttributeName value:[UIFont systemFontOfSize:SKScreenWidth > 320 ? 14 : 13] range:NSMakeRange(0, self.alertTitle.length)];
        ...
        UILabel *label = [[UILabel alloc] initWithFrame:CGRectMake(30, 3, SKScreenWidth - 30 * 2, 52)];
        label.attributedText = string;
        label.numberOfLines = 0;
        [view addSubview:label];
        
        CALayer *layer = [CALayer layer];
        layer.frame = CGRectMake(0, 64 - SKLineHeight, SKScreenWidth, SKScreenHeight);
        [view.layer addSublayer:layer];
        //        layer.backgroundColor = HLSeperatorColor.CGColor;
        return view;
    } else {
        UIView *view = [[UIView alloc] initWithFrame:CGRectMake(0, 0, SKScreenWidth, 5)];
        view.backgroundColor = SKSeperatorColor;
        return view;
    }
}
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    cell.textLabel.font = [UIFont systemFontOfSize:SKScreenWidth > 320 ? 16 : 15];
    if(indexPath.section == 0) {
        NSString *subTitle = self.subTitlesArray[indexPath.row];
        if ([subTitle containsString:@"删除"] || [subTitle containsString:@"delete"]) {
            cell.textLabel.textColor = SKRedColor;
        } else if ([subTitle containsString:@"退出登录"]){
            cell.textLabel.textColor = SKRedColor;
        }
        else {
            cell.textLabel.textColor = SKCommonBlackColor;
        }
...
- (CGFloat)tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath {
    return SKScreenWidth > 320 ? 55 : 50.f;
}
...
- (void)show {
    ...
    self.backgroundView.alpha = 0.0;
    self.tableView.frame = CGRectMake(0, SKScreenHeight, SKScreenWidth, [self heightForTableView]);
...
- (CGFloat)heightForTableView {
    CGFloat rowHeight = SKScreenWidth > 320 ? 55 : 50;
    return  rowHeight * (self.subTitlesArray.count + 1) + 5.0f + ([SKValidStr(self.alertTitle) length] != 0 ? 60 : 0);
}
