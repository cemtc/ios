...
@implementation SKCustomAlertView
...
- (void)setup {
    ...
    _blurView = [[FXBlurView alloc] initWithFrame:CGRectMake(0, 0, SKScreenWidth, SKScreenHeight)];
    ...
    tip.font = [UIFont systemFontOfSize:SKScreenWidth > 320 ? 16 : 15];
    tip.textColor = SKCommonBlackColor;
    
    UILabel *contentLabel = [[UILabel alloc] init];
    NSMutableAttributedString *attributedText = [[NSMutableAttributedString alloc] initWithString:SKValidStr(self.title.length ? self.title : @"")];
    [attributedText addAttribute:NSForegroundColorAttributeName value:[SKCommonBlackColor colorWithAlphaComponent:0.8] range:NSMakeRange(0, attributedText.length)];
    ...
    [attributedText addAttribute:NSFontAttributeName value:SKFont(SKScreenWidth > 320 ? 15 : 14) range:NSMakeRange(0, attributedText.length)];
    contentLabel.attributedText = attributedText;
    
    [self.contentView addSubview:contentLabel];
    _contentLabel = contentLabel;
    contentLabel.textAlignment = NSTextAlignmentCenter;
    contentLabel.textColor = SKCommonBlackColor;
    ...
    [cancel setTitleColor:SKCommonBlackColor forState:UIControlStateNormal];
    [cancel setBackgroundImage:[SKUtils imageWithColor:[[UIColor whiteColor] colorWithAlphaComponent:0.3]] forState:UIControlStateNormal];
    [cancel setBackgroundImage:[SKUtils imageWithColor:[[UIColor lightTextColor] colorWithAlphaComponent:0.3]] forState:UIControlStateHighlighted];
    
    [self.contentView addSubview:cancel];
    _cancelBtn = cancel;
    
    UIView *line = [[UIView alloc] init];
    line.backgroundColor = SKSeperatorColor;
    ...
    [sure setTitleColor:SKRedColor forState:UIControlStateNormal];
    [sure setBackgroundImage:[SKUtils imageWithColor:[[UIColor whiteColor] colorWithAlphaComponent:0.3]] forState:UIControlStateNormal];
    [sure setBackgroundImage:[SKUtils imageWithColor:[[UIColor lightTextColor] colorWithAlphaComponent:0.3]] forState:UIControlStateHighlighted];
    ...
    _btnLine = btnLine;
    btnLine.backgroundColor = SKLightGrayColor;
...
- (void)layoutSubviews {
    ...
    [self.contentView setFrame:CGRectMake(0, 0, SKScreenWidth * 0.7, 150)];
    ...
    if (self.cancel.length == 0 && self.sure.length != 0) {
        ...
        self.btnLine.frame = CGRectMake(self.cancelBtn.right, self.line.bottom + 10, SKLineHeight, 30);
...
- (void)showInView:(UIView *)view {
    ...
    BOOL isShowing = [SKCustomAlertView alertIsShowingInView:view];
    if (isShowing) return ;
    ...
    [SKAnimationManager addTransformAnimationForView:self.contentView];
