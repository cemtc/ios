...
#import "SKCustomSlideViewController.h"

@interface SKCustomSlideViewController () <UIScrollViewDelegate>
...
@implementation SKCustomSlideViewController
...
- (void)setSeletedIndex:(NSInteger)seletedIndex {
    ...
    [self.scrollView setContentOffset:CGPointMake(SKScreenWidth * seletedIndex, 0) animated:YES];
}
...
- (void)scrollViewDidEndDecelerating:(UIScrollView *)scrollView {
    if ([self.delgate respondsToSelector:@selector(customSlideViewController:slideIndex:)]) {
        [self.delgate customSlideViewController:self slideIndex:scrollView.contentOffset.x / SKScreenWidth];
    }
}
