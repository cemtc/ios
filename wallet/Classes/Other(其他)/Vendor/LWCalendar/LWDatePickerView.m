...
-(void)confirmOnClick{
    ...
    if (self.endDate == nil) {
        ...
        [MBProgressHUD showMessage:@"请选择结束日期" toView:nil];
        return;
    }
    
    long long TomorrowNum = [SKUtils getDurationStartTime:[SKUtils datestrFromDate:[NSDate date] withDateFormat:@"yyyy-MM-dd HH:mm:ss"] endTime:[SKUtils datestrFromDate:self.endDate withDateFormat:@"yyyy-MM-dd HH:mm:ss"]];
    if (TomorrowNum > 0) {
        [MBProgressHUD showMessage:@"不能选择未来时间" toView:nil];
        return;
    }
    
    //add不能选择当天时间
    if (self.dialogBuilder.isSelectToday) {
        NSString *today = [SKUtils datestrFromDate:[NSDate date] withDateFormat:@"yyyy-MM-dd"];
        NSString *endday = [SKUtils datestrFromDate:self.endDate withDateFormat:@"yyyy-MM-dd"];
        if ([today isEqualToString:endday]) {
            [MBProgressHUD showMessage:@"不能选择当天时间" toView:nil];
            return;
        }
    }
     ...
     long long mm = [SKUtils getDurationStartTime:[SKUtils datestrFromDate:self.startDate withDateFormat:@"yyyy-MM-dd HH:mm:ss"] endTime:[SKUtils datestrFromDate:self.endDate withDateFormat:@"yyyy-MM-dd HH:mm:ss"]];
    // 秒转天数
    NSInteger days = mm / 3600 / 24;
    if (days > 30) {
        [MBProgressHUD showMessage:@"选择日期间隔不能超过30天" toView:nil];
        return;
    }
...
-(LWDateIndicator *)fromIndicator{
    if (_fromIndicator == nil) {
        _fromIndicator = [LWDateIndicator initWithTitle:@"开始日期" Date:self.dialogDelegate.startDate Delegate:self];
...
-(LWDateIndicator *)toIndicator{
    if (_toIndicator == nil) {
        _toIndicator = [LWDateIndicator initWithTitle:@"结束日期" Date:self.dialogDelegate.endDate Delegate:self];
