...
@implementation QYCandyController{
...
- (void)loadDataRefreshIndex:(NSInteger)index{
    ...
    SKDefineWeakSelf;
    [CFQCommonServer cfqServerQYAPIgetCustomerAwardListWithInput:input Callback:^(id  _Nonnull response, BOOL success, NSString * _Nonnull message, NSString * _Nonnull code) {
...
- (void)setUpItems {
    ...
    self.navigationItem.title = @"糖果记录";
}
...
- (SKBaseTableViewCell *)sk_cellAtIndexPath:(NSIndexPath *)indexPath {
    ...
    QYCandyListCell *cell = [QYCandyListCell cellWithTableView:self.tableView];
