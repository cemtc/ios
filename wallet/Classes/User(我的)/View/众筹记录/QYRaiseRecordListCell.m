//
//  QYRaiseRecordListCell.m
//  wallet
//
//  Created by talking　 on 2019/7/1.
//  Copyright © 2019 talking　. All rights reserved.
//

#import "QYRaiseRecordListCell.h"
#import "CFQLabelBgView.h"
#import "UIView+Layer.h"

@interface QYRaiseRecordListCell ()
...
@implementation QYRaiseRecordListCell
...
- (void)layoutSubviews {
    ...
    [self.topLabel1 mas_makeConstraints:^(MASConstraintMaker *make) {
        ...
        make.top.mas_equalTo(self.topImageView.mas_top).offset(12);
    }];
...
//    self.shouxufeiL.text = [NSString stringWithFormat:@"汇率%@%@",model.currentRage,@"%"];
    ...
    self.leftContentLabel.title = [NSString stringWithFormat:@"%@ %@",model.placementValue,model.coinTypeDesc];
    ...
    NSString *staString = @"发起众筹";
    if ([model.placementStatus isEqualToString:@"1"]) {
        staString = @"发起众筹";
        self.bottomRightLabel.textColor = SKColor(222, 188, 19, 1.0);
        
    }else if ([model.placementStatus isEqualToString:@"2"]){
        staString = @"众筹中";
        self.bottomRightLabel.textColor = SKColor(222, 188, 19, 1.0);
        
    }else if ([model.placementStatus isEqualToString:@"3"]){
        staString = @"失败";
        self.bottomRightLabel.textColor = SKColor(255, 73, 0, 1.0);
        
    }else if ([model.placementStatus isEqualToString:@"4"]){
        staString = @"成功";
