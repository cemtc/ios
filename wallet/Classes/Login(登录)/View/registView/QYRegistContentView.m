...
@implementation QYRegistContentView
...
//    self.passwordTextField1.keyboardType = UIKeyboardTypePhonePad;
    ...
    self.passwordTextField2 = [UITextField textFieldWithPlaceholder:@"请确认登录密码" leftIconImage:SKImageNamed(@"icon_password") height:SKAppAdapter(44) leftViewWith:35];
...
//    self.passwordTextField2.keyboardType = UIKeyboardTypePhonePad;
    ...
    self.nextButton = [UIButton buttonWithTitle:@"下一步" titleColor:SKWhiteColor bgColor:QYThemeColor fontSize:20 target:self action:@selector(nextButtonClick:)];
...
- (void)nextButtonClick:(UIButton *)button {
    ...
    if ([self.delegate respondsToSelector:@selector(qyRegistContentView:didClickInfo:)]) {
        [self.delegate qyRegistContentView:self didClickInfo:@{@"name":@"caofuqing"}];
    }
