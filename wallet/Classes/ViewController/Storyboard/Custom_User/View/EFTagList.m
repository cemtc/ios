...
- (void)setScaleTagInSort:(CGFloat)scaleTagInSort
{
    if (_scaleTagInSort < 1) {
        @throw [NSException exceptionWithName:@"Error" reason:@"(scaleTagInSort)缩放比例必须大于1" userInfo:nil];
    }
...
{
    Class tagClass = _tagClass?_tagClass : [EFTagButton class];
    
    // 创建标签按钮
    EFTagButton *tagButton = [tagClass buttonWithType:UIButtonTypeCustom];
    if (_tagClass == nil) {
    ...
    [tagButton setBackgroundColor:[UIColor colorWithRed:0/255.0f green:148/225.0f blue:228/225.0f alpha:1]];
...
{
    // 获取对应的标题按钮
    EFTagButton *button = self.tags[tagStr];
...
- (void)updateTagButtonFrame:(NSInteger)i extreMargin:(BOOL)extreMargin
{
    ...
    // 获取当前按钮
    EFTagButton *tagButton = self.tagButtons[i];
    // 判断是否设置标签的尺寸
