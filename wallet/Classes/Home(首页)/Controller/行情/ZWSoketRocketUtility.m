...
@interface ZWSoketRocketUtility()<SRWebSocketDelegate>
{
    NSTimer * heartBeat;
...
@end
@implementation ZWSoketRocketUtility
+(ZWSoketRocketUtility *)instance{
    static ZWSoketRocketUtility *Instance = nil;
    static dispatch_once_t predicate;
    dispatch_once(&predicate, ^{
        Instance = [[ZWSoketRocketUtility alloc] init];
    });
    return Instance;
}
...
-(void)SRWebSocketOpen{
    ...
    //wss://www.ccm.one/wss2000/
    self.socket = [[SRWebSocket alloc] initWithURLRequest:
                   [NSURLRequest requestWithURL:[NSURL URLWithString:@"wss://www.ccm.one/wss2000/"]]];
...
- (void)webSocketDidOpen:(SRWebSocket *)webSocket {
    ...
    if (webSocket == self.socket) {
        NSLog(@"************************** socket 连接成功************************** ");
        [self sendData:@{} withRequestURI:@"wss://www.ccm.one/wss2000/"];
    }
}
...
- (void)initHeartBeat
{
    dispatch_main_async_safe(^{
        [self destoryHeartBeat];
        heartBeat = [NSTimer timerWithTimeInterval:60 target:self selector:@selector(ping) userInfo:nil repeats:YES];
...
- (void)sendData:(NSDictionary *)paramDic withRequestURI:(NSString *)requestURI{
    ...
    configDic = @{@"type_id":@"13",@"name":@"0",@"price":@"0",@"zd":@"0",@"cn":@"CNY",@"state":@"2",@"me_member_id":@"0",@"xid":@""};
    NSLog(@"发送的数据--configDic --------------- %@",configDic);
    NSError *error;
    NSString *data;
