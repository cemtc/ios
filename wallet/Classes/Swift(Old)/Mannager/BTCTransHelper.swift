...
public struct BTCTransHelper {
    
    public static func createUtxo(lockScript: Script,value: UInt64,txhash:String, outindex:UInt32) -> UnspentTransaction {
        let txidData =  Data.init() //Data(hex: txhash)
        let txHash: Data = Data(txidData.reversed())
        let outputMock = TransactionOutput(value: value, lockingScript: lockScript.data)
        let outpointMock = TransactionOutPoint(hash:txHash, index: UInt32(outindex))
