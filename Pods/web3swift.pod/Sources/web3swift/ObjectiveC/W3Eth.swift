...
@objc public class W3Eth: NSObject {
    ...
    @objc public func getethTransactionCount(address: W3Address, onBlock: String = "latest") throws -> W3UInt {
        return try swift.getTransactionCount(address: address.swift, onBlock: onBlock).objc
    }
    ...
    */
    ...
    @objc public func getccmBalancePromise(address: W3Address, onBlock: String = "latest", completion: @escaping (W3UInt?,Error?)->()) {
        swift.getccmBalancePromise(address: address.swift, onBlock: onBlock)
            .done { completion($0.objc,nil) }
            .catch { completion(nil, $0) }
    }
