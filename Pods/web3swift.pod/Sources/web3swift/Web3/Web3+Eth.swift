...
public class Web3Eth: Web3OptionsInheritable {
    ...
    public func getethTransactionCount(address: Address, onBlock: String = "latest") throws -> BigUInt {
        return try getethTransactionCountPromise(address: address, onBlock: onBlock).wait()
    }
	/// - Parameter onBlock: Field determines if value is returned based on the state of a blockchain on the latest mined block ("latest")
    /// or the expected state after all the transactions in memory pool are applied ("pending").
    ...
    public func getccmBalance(address: Address, onBlock: String = "latest") throws -> BigUInt {
        print("1请求CCM余额")
        return try getccmBalancePromise(address: address, onBlock: onBlock).wait()
    }
    ...
    public func getccmBalancePromise(address: Address, onBlock: String = "latest") -> Promise<BigUInt> {
        print("2请求CCM余额")
        let request = JsonRpcRequest(method: .getccmBalance, parameters: address._address.lowercased(), onBlock)
        let rp = web3.dispatch(request)
        let queue = web3.requestDispatcher.queue
        return rp.map(on: queue) { response in
            guard let value: BigUInt = response.getValue() else {
    ...
    public func getethTransactionCountPromise(address: Address, onBlock: String = "latest") -> Promise<BigUInt> {
        let addr = address.address
        return getTransactionCountPromise(address: addr, onBlock: onBlock)
    }
