...
public extension Web3.Eth {
    ...
    public func getethTransactionCount(address: EthereumAddress, block: EthereumQuantityTag) -> Promise<EthereumQuantity> {
        return Promise { seal in
            self.getethTransactionCount(address: address, block: block) { response in
                response.sealPromise(seal: seal)
            }
        }
    }
    ...
    public func sendethRawTransaction(transaction: EthereumSignedTransaction) -> Promise<EthereumData> {
        return Promise { seal in
            self.sendethRawTransaction(transaction: transaction) { response in
                response.sealPromise(seal: seal)
            }
        }
    }
