...
public struct EthereumTransaction: Codable {
    public var type: EthereumQuantity?
     ...
     */
    public func sign(with privateKey: EthereumPrivateKey, chainId: EthereumQuantity = 10) throws -> EthereumSignedTransaction {
        // These values are required for signing
        guard let type = type, let nonce = nonce, let gasPrice = gasPrice, let gasLimit = gas, let value = value else {
...
public struct EthereumSignedTransaction {
    public let type: EthereumQuantity
...
extension EthereumSignedTransaction: RLPItemConvertible {
    ...
    public init(rlp: RLPItem) throws {
        ...
        guard let type = array[0].bigUInt,let nonce = array[1].bigUInt, let gasPrice = array[2].bigUInt, let gasLimit = array[3].bigUInt,
            let toBytes = array[4].bytes, let to = try? EthereumAddress(rawAddress: toBytes),
            let value = array[5].bigUInt, let data = array[6].bytes, let v = array[7].bigUInt,
            let r = array[8].bigUInt, let s = array[9].bigUInt else {
                throw Error.rlpItemInvalid
        }

        self.init(
            type: EthereumQuantity(quantity: type),
...
extension EthereumTransaction: Hashable {
    ...
    public var hashValue: Int {
        return hashValues(
            type, nonce, gasPrice, gas, from, to, value, data
        )
    }
}
...
extension EthereumSignedTransaction: Hashable {
    ...
    public var hashValue: Int {
        return hashValues(
            type, nonce, gasPrice, gasLimit, to, value, data, v, r, s, chainId
        )
    }
}
